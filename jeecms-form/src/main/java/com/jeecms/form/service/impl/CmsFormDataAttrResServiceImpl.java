...
public class CmsFormDataAttrResServiceImpl extends BaseServiceImpl<CmsFormDataAttrResEntity, CmsFormDataAttrResDao,Integer> implements CmsFormDataAttrResService {
    ...
    public void uploadDoc(List<Integer> dataAttrIds, CmsSite site) throws GlobalException {
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e1) {
            e1.printStackTrace();
        }
        for(Integer dataAttrId:dataAttrIds){
            CmsFormDataAttrResEntity data = findById(dataAttrId);
            String fileUrl = "";
            if(data!=null){

...
//                }
        ...
        public CopyOnWriteArraySet<CmsDataPerm> getDataPermsByType(Short type, Integer siteId, Short operator,
                ...
                if (type != null&&!dataPerms.isEmpty()) {
                        Set<CmsDataPerm> set = dataPerms.stream().filter(p ->p!=null)
                                .filter(p -> type.equals(p.getDataType()))
                            .filter(p -> (p.getHasDeleted()==null)||(p.getHasDeleted()!=null&&!p.getHasDeleted())).collect(Collectors.toSet());
                        dataPerms.clear();

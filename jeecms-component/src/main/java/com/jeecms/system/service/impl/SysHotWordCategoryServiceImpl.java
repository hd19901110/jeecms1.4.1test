...
public class SysHotWordCategoryServiceImpl extends BaseServiceImpl<SysHotWordCategory, SysHotWordCategoryDao, Integer> implements SysHotWordCategoryService {
    ...
    @Override
    public void deleteByIds(Integer[] ids, Integer siteId) throws GlobalException {
        List<SysHotWordCategory> allById = findAllById(Arrays.asList(ids));
        Integer[] categoryIds = new Integer[allById.size()];
        for (int i = 0; i < allById.size(); i++) {
                categoryIds[i] = allById.get(i).getId();
        }
        hotWordService.delete(categoryIds, siteId);
        physicalDelete(ids);
    }

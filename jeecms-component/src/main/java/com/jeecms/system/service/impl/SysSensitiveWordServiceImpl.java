...
public class SysSensitiveWordServiceImpl extends BaseServiceImpl<SysSensitiveWord, SysSensitiveWordDao, Integer> implements SysSensitiveWordService {
	...
	public ResponseInfo saveBatch(SensitiveWordDto dto, String userName) throws GlobalException {
		...
		List<SysSensitiveWord> updateList = new ArrayList<>();
		for (String str : set) {
			if (StringUtils.isBlank(str)) {
				continue;
			}
			SysSensitiveWord sysSensitiveWord = dao.findBySensitiveWord(str);
			//不存在则新增
			if (sysSensitiveWord == null) {
				...
				if (SensitiveWordDto.DEAL_WITH_COVER.equals(dto.getDealWithType())) {
					sysSensitiveWord.setReplaceWord(dto.getReplaceWord());
					sysSensitiveWord.setCreateTime(Calendar.getInstance().getTime());
					sysSensitiveWord.setCreateUser(userName);
					updateList.add(sysSensitiveWord);
				}
			}
		}
		if (list.size() > 0) {

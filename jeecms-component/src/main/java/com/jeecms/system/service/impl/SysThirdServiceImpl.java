...
public class SysThirdServiceImpl extends BaseServiceImpl<SysThird, SysThirdDao, Integer> implements SysThirdService {
	...
	@Override
	public void saveOrUpdate(SysThird bean) throws GlobalException {
		SysThird third = getCode(bean.getCode());
		if (third == null) {
			save(bean);
		} else {
			third.setReMark(bean.getReMark());
			third.setIsEnable(bean.getIsEnable());
			third.setAppId(bean.getAppId());
			third.setAppKey(bean.getAppKey());
			update(third);
		}
	}
}
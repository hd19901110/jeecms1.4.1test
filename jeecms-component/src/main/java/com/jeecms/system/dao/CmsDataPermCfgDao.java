...
public interface CmsDataPermCfgDao extends IBaseDao<CmsDataPermCfg, Integer> {
        ...
        @Modifying
        @Query(value = "delete from CmsDataPermCfg permCfg where permCfg.orgId=?1")
        void deleteByOrgId(Integer orgId);

        @Modifying
        @Query(value = "delete from CmsDataPermCfg permCfg where permCfg.roleId=?1")
        void deleteByRoleId(Integer roleId);

        @Modifying
        @Query(value = "delete from CmsDataPermCfg permCfg where permCfg.userId=?1")

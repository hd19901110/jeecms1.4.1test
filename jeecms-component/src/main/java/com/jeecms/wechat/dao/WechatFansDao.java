...
public interface WechatFansDao extends IBaseDao<WechatFans, Integer>, WechatFansDaoExt {
	...
	@Query("select count(*) from WechatFans bean where 1 = 1 and bean.appId=?1 and "
			+ "bean.subscribeTime>=?2 and bean.subscribeTime<=?3 and bean.hasDeleted=0")
	...
	@Query("select sex,count(1) from WechatFans bean where 1 = 1 and bean.appId=?1 "
			+ "and bean.hasDeleted=0 group by sex order by count(1) desc")
	...
	@Query("select subscribeScene,count(1) from WechatFans bean where 1 = 1 and "
			+ "bean.appId=?1 and bean.hasDeleted=0 group by subscribeScene order by count(1) desc")
	...
	@Query("select country,count(1) from WechatFans bean where 1 = 1 and bean.appId=?1 "
			+ "and bean.hasDeleted=0 group by country order by count(1) desc")
	...
	@Query("select province,count(1) from WechatFans bean where 1 = 1 and bean.appId=?1 "
			+ "and country ='中国' and bean.hasDeleted=0 group by province order by count(1) desc")
	 ...
	 */
	@Query("select count(*) from WechatFans bean where 1 = 1 and bean.appId=?1 and "
			+ "bean.hasDeleted=0 and member_id is not null")
	 ...
	 * @return: Integer 数量
	 */
	@Query("select count(*) from WechatFans bean where 1 = 1 and bean.appId=?1 and bean.hasDeleted=0")

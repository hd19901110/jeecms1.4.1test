...
public class SystemContextUtils {
    ...
    public static boolean replay() {
        ...
        if (request != null) {
            ...
            if (StringUtils.isNotBlank(replay)) {
                isReplay =  Boolean.parseBoolean(replay);
                if (isReplay) {
                    CoreUser user = SystemContextUtils.getUser(request);
                    if (user == null) {
                        return true;
                    }
                    return user.getSafetyAdmin();
                }
            }
        }
        return false;
    ...
    public static String operatingUserName() {
        HttpServletRequest request = RequestUtils.getHttpServletRequest();
        if (request != null) {
            String operatingUserName = request.getHeader(WebConstants.OPERATING_USER_NAME);
            if (StringUtils.isNotBlank(operatingUserName)) {
    ...
    public static Integer getDataPermStatus() {
        HttpServletRequest request = RequestUtils.getHttpServletRequest();
        if (request != null) {
            String dataPermStatus = request.getHeader(WebConstants.DATA_PERM_STATUS);
            if (StringUtils.isNotBlank(dataPermStatus)) {
                return Integer.valueOf(dataPermStatus);
            }
        }
        return null;
    }

    public static Integer getSafeManageStatus() {
        HttpServletRequest request = RequestUtils.getHttpServletRequest();
        if (request != null) {
            String safeManageStatus = request.getHeader(WebConstants.SAFE_MANAGE_STATUS);
            if (StringUtils.isNotBlank(safeManageStatus)) {
                return Integer.valueOf(safeManageStatus);
            }
        }
        return null;
    }

...
//            /**查询子栏目*/
//            List<Channel>ccs= channelService.findByIds(Arrays.asList(channelIds));
//            for(Channel c:ccs){
//                channels.addAll(c.getChildAllIds());
//            }
//        }else {
//            channels.addAll(Arrays.asList(channelIds));
//        }
...
//            }
        ...
        List<Content> contents = dao.getList(channelOption,channelIds, tagIds, channelPaths, siteId, typeIds, title, date,
                releaseTarget, isTop, timeBegin, timeEnd, excludeId, modelId, orderBy, count);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class,readOnly = true)
    public ContentContributeVo splicContributeVo(Integer contentId,Integer userId)  throws GlobalException{
        ContentContributeVo contentContributeVo = dao.findContributoVo(contentId);

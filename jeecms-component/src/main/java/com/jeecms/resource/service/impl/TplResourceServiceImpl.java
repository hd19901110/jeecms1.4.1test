...
public class TplResourceServiceImpl implements TplResourceService {
	...
	public void copyTplAndRes(CmsSite from, CmsSite to) throws IOException {
		...
		File tplTo = new File(realPathResolver.get(to.getTplPath()));
		if (tplFrom.exists() && !tplFrom.getPath().equals(tplTo.getPath())) {
			FileUtils.copyDirectory(tplFrom, tplTo);
			File resFrom = new File(realPathResolver.get(from.getResourcePath()));
			if (resFrom.exists()) {
				File resTo = new File(realPathResolver.get(to.getResourcePath()));

...
public class TplServiceImpl implements TplService {
    ...
    @Override
    public List<String> getIndexBySolutionPrefixPath(String solution, String prefix, CmsSite site) throws IOException, GlobalException {
        String url = site.getTplPrefix(solution, prefix);
        List<Tpl> list = getListByPrefix(url, site);
        List<String> result = new ArrayList<String>(list.size());
        for (Tpl tpl : list) {
            result.add(tpl.getName());
    ...
    public void rename(String orig, String dist, CmsSite site) throws GlobalException {
        ...
        try {
            if (of.isDirectory()) {
                ...
                String distSolution = "";
                if (distSpt != -1) {
                    distSolution = distRep.substring(0, distSpt);
                }
                //模板方案名不为空
                if (StringUtils.isNotBlank(solution)) {
                    name = name.replace(solution + SPT, "");
    ...
    public void update(TplUpdateDto dto, CmsSite site) throws GlobalException {
        ...
        try {
            ...
            if (!name.equals(newName)) {
                //修改文件名称
                String os = realPathResolver.get(name);
                File of = new File(os);
                String ds = realPathResolver.get(newName);
                File df = new File(ds);
                if (of.isDirectory()) {
                    FileUtils.moveDirectory(of, df);
                } else {
            ...
            //先删除原来的数据再添加
            String tplPath = site.getTplPath();
            String fname = name.replace(tplPath + WebConstants.SPT, "");
            ...
            modelTplService.physicalDeleteInBatch(list);
            //添加模板模型关联
            modelTplService.saveBatch(newName.substring(0, newName.lastIndexOf(SPT)), site, newName, modelIds);
        } catch (IOException e) {

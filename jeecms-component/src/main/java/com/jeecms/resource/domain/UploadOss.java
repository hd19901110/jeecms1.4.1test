...
public class UploadOss extends AbstractDomain<Integer> implements Serializable {
         ...
         */
        @Transient
        public List<String> child(String remote) {
                List<String> list = new ArrayList<>(16);
                if (OSS_TYPE_Q_CLOUD.equals(getOssType())) {
                        return QcoscloudClient.parseList(getBucketArea(), getBucketName(), getAppId(), getSecretId(),
                                getAppKey(), remote);
                } else if (OSS_TYPE_ALI_CLOUD.equals(getOssType())) {
                        return AliOssCloudClient.parseList(getBucketName(), getEndPoint(), getAppKey(),
                                getSecretId(), remote.substring(1));
                } else if (OSS_TYPE_QINIU_CLOUD.equals(getOssType())) {
                        return  QiniuOssCloudClient.parseList(getBucketName(),
                                getAppKey(), getSecretId(), remote.substring(1));
                }
                return list;
        }

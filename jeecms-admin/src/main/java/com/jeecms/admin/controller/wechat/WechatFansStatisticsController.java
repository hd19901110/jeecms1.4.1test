	...
	* @Title: sort 
	...
	public List<WechatFansStatistics> sort(List<WechatFansStatistics> list, Integer sortType, Boolean order) {
		if (sortType != null && order != null) {
			//日期,true正序 false倒序
			if (sortType.equals(WechatFansStatistics.ORDER_TYPE_DATE)) {
				if (order) {
			...
			} else if (sortType.equals(WechatFansStatistics.ORDER_TYPE_NEW)) {
				if (order) {
					list = list.stream().sorted(Comparator
			...
			} else if (sortType .equals(WechatFansStatistics.ORDER_TYPE_QUIT) ){
				if (order) {
					list = list.stream().sorted(Comparator
			...
			} else if (sortType .equals(WechatFansStatistics.ORDER_TYPE_NORMAL)) {
				if (order) {
					list = list.stream().sorted(Comparator
			...
			} else if (sortType.equals(WechatFansStatistics.ORDER_TYPE_SUM)) {
				if (order) {
					list = list.stream().sorted(Comparator

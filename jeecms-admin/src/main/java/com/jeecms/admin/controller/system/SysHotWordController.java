...
public class SysHotWordController extends BaseAdminController<SysHotWord, Integer> {
	...
	@SerializeField(clazz = SysHotWord.class, includes = {"id", "hotWord", "linkUrl", "isTargetBlank", "remark",
			"useCount", "clickCount", "cateName", "hotTerm"})
	public ResponseInfo page(HttpServletRequest request, Integer hotTermCategoryId,
							 String hotTerm, @PageableDefault(sort = "createTime",
			direction = Direction.DESC) Pageable pageable) throws GlobalException {
		validateId(hotTermCategoryId);
		...
		params.put("EQ_hotWordCategoryId_Integer", new String[]{String.valueOf(hotTermCategoryId)});
		params.put("LIKE_hotWord_String", new String[]{hotTerm});
	...
	public ResponseInfo checkByHotWord(String hotTerm, Integer id) {
		return new ResponseInfo(service.checkByHotWord(hotTerm, id));
	}

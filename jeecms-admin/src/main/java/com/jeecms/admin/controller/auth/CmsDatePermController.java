...
public class CmsDatePermController extends BaseAdminController<CmsDataPerm, Integer> {
	...
	public ResponseInfo assembleData(Integer orgId, Integer roleId, Integer userId, Integer siteId, Short dataType)
		...
		CmsDataPermVo vo = dataPermPlusService.assembleData(orgId, roleId, userId, siteId, dataType, currUser);
	...
	public ResponseInfo update(HttpServletRequest request, @RequestBody @Valid CmsDatePermDto dto, BindingResult result)
		...
		if (dto.getOrgId() != null) {
			CmsOrg org = orgService.findById(dto.getOrgId());
			org.validManagerAble();
		}else if (dto.getRoleId() != null) {
			CoreRole role = roleService.findById(dto.getRoleId());
			role.validManagerAble();
		}else if (dto.getUserId() != null) {
			CoreUser user = userService.findById(dto.getUserId());

...
import static com.jeecms.common.util.FrontUtilBase.PAGE_NOT_FOUND;
...
public class ExceptionHandlerController {
    ...
    public String handlerException404(HttpServletRequest request, HttpServletResponse response, Exception e,
        ...
        commonError404(request,response,e,model);
        return FrontUtils.pageNotFound(request, response, model);
    }

    @RequestMapping(value = { WebConstants.TPL_404 })
    public String handlerTplException404(HttpServletRequest request, HttpServletResponse response, Exception e,
                                         ModelMap model) throws GlobalException, IOException {
        commonError404(request,response,e,model);
        return FrontUtils.getSysPagePath(request,TPLDIR_COMMON,PAGE_NOT_FOUND);
    }

    private void commonError404(HttpServletRequest request, HttpServletResponse response, Exception e,
                                ModelMap model)  throws GlobalException, IOException{
        ...
        /** 资源不返回数据 */
        if (StringUtils.isNotBlank(errorRequestUri) && UrlUtil.isResourceUri(errorRequestUri)) {
            return;
        }
        ...
        if(errorRequestUri!=null&&(errorRequestUri.startsWith("/"+WebConstants.STATIC_PC_PATH)

...
import com.jeecms.common.wechat.bean.request.open.*;
...
public class WechatController {
	...
	@ResponseBody
	public ResponseInfo getWxCode(String url,HttpServletRequest request,
								HttpServletResponse response, ModelMap model) {
		AbstractWeChatOpen config=weChatOpenService.findOpenConfig();
		/**未取到咋取主站点的*/
		AbstractWeChatInfo weChatInfo = weChatInfoService.findDefault(AbstractWeChatInfo.TYPE_WECHAT, SystemContextUtils.getSiteId(request));
		if(weChatInfo==null){
			 weChatInfo = weChatInfoService.findDefault(AbstractWeChatInfo.TYPE_WECHAT, 1);
		}
		CmsSite site = SystemContextUtils.getSite(request);
		String redirectUrl = site.getUrlDynamicForFix(true)+"weChat/setOpenId";
		String tokenUrl=weixinAuthCodeUrl+"&appid="+weChatInfo.getAppId()+"&component_appid="+config.getAppId()+"&redirect_uri="+redirectUrl+"#wechat_redirect";
		HttpClientUtil.get(tokenUrl);
		Object openId = (Object)sessionProvider.getAttribute(request, WebConstants.WECHAT_OPEN_ID);
		Map<String,String>map = new HashMap<>();
		LOGGER.info("getWxCode url->"+url);
		sessionProvider.setAttribute(request, "lastUrl",url);
    ...
    public void setOpenId(String code,HttpServletRequest request,
        ...
        AbstractWeChatToken weChatToken = weChatTokenService.findByAppId(config.getAppId());

        ValidateToken validatoken = new ValidateToken();
        /**公众号开放平台的appid*/
        validatoken.setAppId(config.getAppId());
        validatoken.setAccessToken(weChatToken.getComponentAccessToken());

        GetOpenIdRequest getOpenIdRequest = new GetOpenIdRequest();
        /**公众号的appid*/
        getOpenIdRequest.setAppid(weChatInfo.getAppId());
        getOpenIdRequest.setCode(code);
        getOpenIdRequest.setComponentAppid(config.getAppId());
        getOpenIdRequest.setComponentAccessToken(weChatToken.getComponentAccessToken());
        String openId ="";
        try {
            GetOpenIdResponse getOpenIdResponse = oauthApiService.getOpenId(getOpenIdRequest,validatoken);
            LOGGER.info("openid->"+getOpenIdResponse.getOpenid());
            sessionProvider.setAttribute(request, WebConstants.WECHAT_OPEN_ID,getOpenIdResponse.getOpenid());
            openId = getOpenIdResponse.getOpenid();
        }catch (GlobalException e){
            LOGGER.error(e.getMessage());
        }
        Object url = (Object)sessionProvider.getAttribute(request, "lastUrl");
        LOGGER.info("setOpenId url->"+url);
        try {
            String urlStr ;
            if(url==null){
                urlStr = SystemContextUtils.getSite(request).getUrlWhole();
            }else{
                urlStr = (String) url;
            }
            LOGGER.info("redirect url->"+urlStr);
            response.sendRedirect(urlStr);
        } catch (IOException e) {
            e.printStackTrace();
        }
        //return new ResponseInfo(openId);
    }


    @Value("${weixin.auth.getCodeUrl}")
    private String weixinAuthCodeUrl;

    @Autowired
    AbstractWeChatOpenService weChatOpenService;
    @Autowired
    WeChatLoginApiService weChatLoginApiService;
    @Autowired

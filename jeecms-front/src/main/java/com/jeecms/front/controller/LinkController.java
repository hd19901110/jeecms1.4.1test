...
public class LinkController {
	...
	@GetMapping("/list")
	@MoreSerializeField({@SerializeField(clazz = Link.class, includes = {"id", "linkName", "linkUrl", "isEnable",
		"remark", "linkTypeId", "siteId", "linkType", "linkTypeName", "resourcesSpaceData"}),
		@SerializeField(clazz = ResourcesSpaceData.class, includes = {"url"})})
	public ResponseInfo list(HttpServletRequest request) {
		Integer siteId = SystemContextUtils.getSiteId(request);
		//获取站点下友情链接
		List<Link> links = linkService.findList(siteId);
		//根据友情链接类别分类

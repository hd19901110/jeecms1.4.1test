...
public class SingletonEhCacheRegionFactory implements RegionFactory {
	...
	/**插件缓存配置key*/
	public static final String NET_SF_EHCACHE_PLUG_CONFIGURATION_RESOURCE_NAME_PREFIX = "net.sf.ehcache.configurationResourceNamePrefix";
	...
	public void start(SessionFactoryOptions settings, Properties properties) throws CacheException {
		...
		try {
			...
			if (configurationResourceName == null || configurationResourceName.length() == 0) {
				...
				/**加载插件的配置*/
				if (properties != null) {
					List<String>configResourceNames =  PropertiesUtil.getList(properties,NET_SF_EHCACHE_PLUG_CONFIGURATION_RESOURCE_NAME_PREFIX);
					Configuration plugConfig = null;
					for(String rn:configResourceNames){
						URL rurl;
						try {
							rurl = new URL(rn);
						} catch (MalformedURLException e) {
							if (!configurationResourceName.startsWith(SPT)) {
								rn = SPT + rn;
								LOG.debugf("prepending / to %s. It should be placed in the "
										+ "root of the classpath rather than in a package.", rn);
							}
							rurl = loadResource(rn);
							plugConfig = HibernateEhcacheUtils.loadAndCorrectConfiguration(rurl);
							for(CacheConfiguration ck:plugConfig.getCacheConfigurations().values()){
								configuration.addCache(ck);
							}
						}
					}
				}

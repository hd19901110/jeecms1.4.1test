...
public class QcoscloudClient {
	 ...
	 * @param appKey
	 * @param cosFilePath
	 * @param path 上传目录（加 / 号）类似 /xx/
	 * @return
	 */
	public static List<String> parseList(String bucketArea, String bucketName,
										 String appId, String secretId, String appKey, String path) {
		String code = "code";
		String data = "data";
		String infos = "infos";
		String name = "name";
		...
		String folder = listFolder(bucketArea, bucketName,appId, secretId, appKey, path);
		if(StringUtils.isNotBlank(folder)){
			JSONObject obj = JSON.parseObject(folder);
			if (obj.getInteger(code) == 0 && obj.getJSONObject(data) != null) {
				JSONArray jsonArray = obj.getJSONObject(data).getJSONArray(infos);
				//判断路径是否存在
				for (int i = 0; i < jsonArray.size(); i++) {
					String p = jsonArray.getJSONObject(i).getString(name);
					//不为空且不包含"/",则为文件
					if (StringUtils.isNotBlank(p) && !p.contains("/")) {
						list.add(p.substring(0, p.lastIndexOf(".")));
					}
				}
			}
		}

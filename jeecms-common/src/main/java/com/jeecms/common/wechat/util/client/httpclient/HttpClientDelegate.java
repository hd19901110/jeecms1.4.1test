...
public class HttpClientDelegate extends AbstractHttpClient implements HttpDelegate {
	...
	public String get(String url, Map<String, String> params) {
		...
		try {
			...
			LOGGER.debug(java.text.Normalizer.normalize(String.format("get request url:%s, result:%s", url, response),
	...
	@Override
	public int getResponseCode(String url, Map<String, String> params) {
		Integer status = 500;
		try {
			//status = HTTP_CLIENT.execute(buildGetRequest(url, params),RESPONSE_CODE_HANDLER);
			status = HTTP_CLIENT.execute(buildHeadRequest(url, params),RESPONSE_CODE_HANDLER);
		} catch (Exception e) {
		}
		return  status;
	}
	...
	@Override
	public void getIgnore(String url, Map<String, String> params) {
		try {
			HTTP_CLIENT.execute(buildGetRequest(url, params), STRING_HANDLER);
